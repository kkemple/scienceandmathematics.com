...
where $p_c = 1/\langle k \rangle = 1$ for random networks. Below threshold, only small isolated components exist. Above threshold, a giant component of non-vanishing size emerges enabling cascades. Near the critical point, correlation length diverges as $\xi \sim |p - p_c|^{-\nu}$ with critical exponent $\nu \approx 0.88$ in three dimensions—the same universality class as liquid-gas transitions. The [constraint eigenvalue framework](/a-constraint-eigenvalue-theory-of-information-matter-and-mind) identifies similar phase transition structure across physical and organizational systems, with the universal exponent $\nu = 1/\rho^* \approx 0.304$ governing coherence-length divergence near critical thresholds.
