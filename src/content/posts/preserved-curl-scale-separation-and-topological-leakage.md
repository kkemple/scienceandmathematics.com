...
pubDate: '2026-02-08'
zenodoDepositionId: 18576048
zenodoUrl: "https://zenodo.org/records/18576048"
doi: "10.5281/zenodo.18576048"
zenodoDescription: |
  Magnetic helicity — the integral measuring linkage and twist of divergence-free field lines — serves as a physical curl inventory whose decay timescale exceeds the energy decay timescale by a factor controlled entirely by scale separation. A Monte Carlo sweep across carrier and dissipative scales produces a universal stiffness collapse onto the linear law τ_H/τ_U ≈ 0.97X + 0.03, where X = (k_rms/k_c)². Introducing stochastic topology-change events produces a phase boundary whose critical leakage strength follows a power law γ_crit ~ X^α with α ∈ [0.75, 1.0] (mean ≈ 0.84), depending on retention strictness — a protection–repair tradeoff where stricter topological preservation reduces the tolerable reconnection rate at high scale separation. The results provide a concrete field-theoretic model for the curl-maintenance functional established in the geometry of self-correction framework, and jointly constrain the repair efficiency function in the repair-augmented RG system.
keywords:
  - "magnetic helicity"
  - "curl"
  - "scale separation"
...
At low $X$ ($\lesssim 3$ for $h_{\min} = 0.6$; $\lesssim 5$ for $h_{\min} = 0.8$), $\gamma_{\mathrm{crit}}$ is censored at the minimum tested value ($\gamma = 0.023$), meaning protection holds even at the lowest leakage rate in the sweep. These points establish only a lower bound. For uncensored points at higher $X$, the phase boundary follows a power law $\gamma_{\mathrm{crit}}(X) = a\,X^\alpha$. Fitting the $k_c = 1$ track (widest $X$ range, six uncensored points spanning $X \in [12, 122]$) gives:

| $h_{\min}$ | $a$ | $\alpha$ | Uncertainty on $\alpha$ |
|:---:|:---:|:---:|:---:|
| 0.6 | 0.024 | 0.96 | ±0.08 |
| 0.7 | 0.016 | 0.81 | ±0.04 |
| 0.8 | 0.020 | 0.75 | ±0.10 |

The exponent $\alpha$ ranges from $0.75$ to $0.96$ depending on the strictness of the retention threshold, with a mean $\bar{\alpha} \approx 0.84$. The pattern is monotonic: stricter protection requirements (higher $h_{\min}$) reduce $\alpha$, meaning systems that demand stronger topological preservation tolerate proportionally less reconnection at high scale separation. The $k_c$ dependence is consistent — at $h_{\min} = 0.7$, fitting across all $k_c$ values with sufficient uncensored points:

| $k_c$ | $\alpha$ | Uncertainty | Points |
|:---:|:---:|:---:|:---:|
| 1 | 0.81 | ±0.04 | 6 |
| 2 | 0.73 | ±0.05 | 5 |
| 3 | 0.70 | ±0.07 | 3 |

Higher $k_c$ (smaller-scale cut, narrower scale gap) gives slightly lower $\alpha$, but the variation is modest. The power-law structure is robust across configurations. This is a protection–repair tradeoff: the same topological rigidity that shields a system from accidental degradation also limits its capacity for deliberate self-modification. The tradeoff is not catastrophic — the exponent stays in $[0.75, 1.0]$, never approaching the $\alpha = 0$ limit where repair would be suppressed independently of $X$ — but it is real and monotonic. The full boundary data are available as [supplementary tables](/data/helicity-2B-phase-boundary.csv).

Three features of this result merit emphasis. Scale separation creates topological protection, and the critical leakage strength scales as a power law $\gamma_{\mathrm{crit}} \sim X^\alpha$ with $\alpha \in [0.75, 1.0]$ depending on retention strictness. Topology change, when it occurs, is punctuated rather than smooth — helicity drops in discrete events rather than eroding continuously, consistent with the reconnection phenomenology observed in both laboratory plasmas and astrophysical contexts[^2]. And protection is conditional: above the critical leakage strength, the circulation inventory collapses regardless of scale separation. Irreducible curl exists, but only within a bounded region of parameter space.
...
The Monte Carlo phase boundary exhibits scatter that reflects both the stochastic nature of the topology-change events and the finite resolution of the bisection search ($\gamma_0$ values are quantized). The power-law fits to $\gamma_{\mathrm{crit}}(X)$ rest on 3–6 uncensored points per track, with low-$X$ points censored at the minimum tested leakage rate. The exponent uncertainties ($\pm 0.04$ to $\pm 0.10$) reflect this limited dynamic range. The trends are robust across all three retention thresholds and multiple $k_c$ values, but individual boundary points should be read as estimates within a factor of roughly two rather than precise critical values.
