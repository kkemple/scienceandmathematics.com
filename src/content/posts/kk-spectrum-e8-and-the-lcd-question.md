...
zenodoDepositionId: 19652103
zenodoUrl: "https://zenodo.org/records/19652103"
doi: "10.5281/zenodo.19652103"
category: 'core'
order: 3
zenodoDescription: |
...
The scalar KK spectrum on $S^3/\mathrm{2I}$ is controlled by a single algebraic object. Fifteen spin levels are forbidden below the Coxeter number, and those fifteen are the gap set of the numerical semigroup $\langle 6, 10, 15 \rangle$ generated by the halved Klein invariant degrees of the $E_8$ singularity. The semigroup has genus $g = 15 = h/2$, a property unique to $E_8$ among all ADE types. The first surviving mode carries a protection factor of 56× above the generic $S^3$ first excitation—the largest among all finite subgroup quotients of $S^3$. The effective spectral dimension $d_{\mathrm{eff}} = \varphi^2$ is exact, mediated by the Chebyshev norm ratio at the two icosahedral half-angles, and the regression agrees with the exact identity to sub-percent precision.

The LCD question—the lowest common denominator between the 6D embedding dimension and the $E_8$ algebraic structure—is that they operate on different channels. 6D is the minimum embedding dimension for the periodic parent lattice that projects to a 3D icosahedral quasicrystal—standard quasicrystallography (Kramer–Neri, 1984[^10]). $E_8$ (whose root lattice lives in 8D) is the algebraic structure governing the compactification geometry; it enters through the McKay correspondence, which identifies finite subgroups of $\mathrm{SU}(2)$ with ADE Dynkin diagrams and determines the spectral content of $S^3/\mathrm{2I}$—which KK modes survive, how the gap set is organized, and what effective dimension the low-energy spectrum carries.

The chain is forced. Negative selection gives $C_{10}$. $C_{10}$ on $S^3$ gives $\mathrm{2I}$. $\mathrm{2I}$ acting on $\mathbb{C}^2$ gives the Klein invariants. The Klein invariants satisfy the $E_8$ surface equation. The McKay correspondence confirms the identification. The spectral content—15 forbidden levels, protection factor 56×, genus equal to $h/2$, Frobenius number 29, effective dimension exactly $\varphi^2$—follows from the invariant ring structure alone. Every link is a theorem.
