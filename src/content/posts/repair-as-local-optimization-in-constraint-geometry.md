...
This paper treats repair as a first-class process in constraint geometry. Repair consists of localized architectural operations—reconfiguration, consolidation, re-routing—that attempt to reduce excess curl generated by history and accumulated structural debt. It is neither global optimization nor inverse renormalization group flow. It is local optimization under geometric constraint, bounded by the same slack $(1 - \xi)$ available to all other system activity and further gated by overhead costs that arise because repair operations themselves must navigate the non-integrable constraint manifold. The analysis identifies two empirically distinct operating modes, captures both under a single gating function, and writes the resulting repair-augmented system as a set of coupled flow equations with explicit parameter dependence.
...
$$
c_{\mathrm{geom}} \geq \kappa \, \lambda_1^{\mathrm{co\text{-}ex}} \, |\beta_{\mathrm{rep}}|^2,
$$

where $\lambda_1^{\mathrm{co\text{-}ex}} > 0$ is the first eigenvalue of the Hodge Laplacian restricted to co-exact 1-forms (the subspace that produces curl; exact forms and harmonics contribute nothing to the curl-maintenance functional), $\beta_{\mathrm{rep}}$ is the co-exact component of the projection defect induced by the repair operation's own implementation, and $\kappa = O(1)$ absorbs normalization and locality conventions.

*Proof sketch.* The argument is compositional. Non-integrability of $\Pi_x$ implies that implemented correction fields generically contain a co-exact component in their Hodge decomposition that cannot be removed by gauge or gradient adjustment. Decompose the projection defect as $\delta\alpha_{\mathrm{rep}} = \delta\alpha_{\mathrm{rep,harm}} + d\psi_{\mathrm{rep}} + \beta_{\mathrm{rep}}$ via the Hodge decomposition into harmonic, exact, and co-exact parts. Harmonic forms are closed ($d\delta\alpha_{\mathrm{rep,harm}} = 0$) and exact forms satisfy $d(d\psi_{\mathrm{rep}}) = 0$, so all curl resides in the co-exact component $\beta_{\mathrm{rep}}$. The coercivity of the exterior derivative on the co-exact subspace is controlled by $\lambda_1^{\mathrm{co\text{-}ex}}$, yielding a sharp lower bound on $\mathcal{M}_{\mathrm{curl}}$ for any feasible field. A repair operation that modifies system architecture while respecting feasibility must itself be realized through $\Pi_x$, and consequently its implementation induces a co-exact projection defect $\beta_{\mathrm{rep}}$ in the same subspace. The same spectral gap bound therefore applies to the repair operation's own implementation cost. Repair may reduce excess curl associated with architectural history, but it cannot avoid paying a strictly positive geometric overhead whenever feasibility remains non-integrable.
...
where $c = c_{\mathrm{geom}} + c_{\mathrm{arch}}$ captures two distinct sources of overhead. The Repair–Geometry Compatibility lemma establishes that the geometric component satisfies $c_{\mathrm{geom}} \geq \kappa \, \lambda_1^{\mathrm{co\text{-}ex}} \, |\beta_{\mathrm{rep}}|^2$—it is bounded below by the co-exact spectral gap that sets the curl floor, scaled by the co-exact component of the projection defect of the repair operation itself. This is not an assumption but a consequence of the fact that repair must navigate the same non-integrable feasibility map that generated the excess curl. The architectural component $c_{\mathrm{arch}} \geq 0$ captures additional overhead from coordination, sequencing, and the system-specific logistics of implementing structural change.
...
Mapping the empirical shutoff at $R/R_S \sim 10^3$ to the repair-capacity analysis uses the compactness–load relationship established in the [triadic tension collapse trajectory](/triadic-tension-decade-symmetry-and-dissipation-flow-in-constraint-geometry), where $R/R_S = 10^3$ corresponds to $M \approx 1.17 \, M_\odot$ with $\xi = 0.46$. This gives
